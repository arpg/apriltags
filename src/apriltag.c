...
#include "apriltags/timeprofile.h"
#include "apriltags/common.h"
...
static void pop_count_fast_setup()
{
    for (int i = 0; i < 4096; i++)
        pop_count_table[i] = pop_count_slow(i);
}
 ...
 **/
static int decode_tag(april_tag_family_t *tf, uint64_t rcode, uint32_t *id, uint32_t *hamming, uint32_t *rotation, uint64_t *rotatedcode, uint32_t guess_id)
{
    ...
    if (guess_id < tf->ncodes) {
        for (int rot = 0; rot < 4; rot++) {
            uint32_t thishamming = hamming_distance(rcodes[rot], tf->codes[guess_id]);
    ...
    for (uint32_t id = 0; id < tf->ncodes; id++) {
        ...
        for (uint32_t rot = 0; rot < 4; rot++) {
            uint32_t thishamming = hamming_distance(rcodes[rot], tf->codes[id]);
            if (thishamming < besthamming) {
                besthamming = thishamming;
                bestrotation = rot;
                bestid = id;
    ...
    if (besthamming < tf->h / 2) {
        ...
        *rotatedcode = tf->codes[bestid];
        for (uint32_t i = 0; i < 4-bestrotation; i++)
...
april_tag_detector_t *april_tag_detector_create(april_tag_family_t *fam)
{
    ...
    april_tag_detector_t *td = (april_tag_detector_t*) calloc(1, sizeof(april_tag_detector_t));
    ...
    td->min_mag = 30; // squared magnitude of gradient. (a'a)
...
static void quad_search(zarray_t *quads, april_tag_detector_t *td, int pos, struct quad *quad)
{
    struct segment *seg = quad->segments[pos];

    if (seg->neighbors == NULL)
        return;
    ...
    for (int i = 0; i < zarray_size(seg->neighbors); i++) {
        ...
        if (neighbor->p0[0] < quad->segments[0]->p0[0])
            continue;

        // the last segment must form a loop with the first segment.
        if (pos==3) {
            if (neighbor != quad->segments[0])
                continue;
        } else {
        ...
        if (0 && pos>=2) {
            ...
            double theta1 = atan2f(quad->p[a][1] - quad->p[b][1],
                                   quad->p[a][0] - quad->p[b][0]);

            double theta0 = atan2f(quad->p[b][1] - quad->p[c][1],
        ...
        if (pos==3) {
            ...
            if (mindist2 < sq(td->min_tag_size))
                continue;
            if (maxdist2 > mindist2 * sq(td->max_aspect_ratio))
                continue;
...
static void neighbor_search_task(void *_u)
{
    ...
    for (int i = task->i0; i < task->i1; i++) {
        ...
        if (length2 < min_segment_length2)
            continue;

        float theta0 = atan2f(seg->p1[1] - seg->p0[1],
                              seg->p1[0] - seg->p0[0]);
        ...
        while (1) {
            ...
            if (dist2 > max_segment_distance2)
                continue;
            ...
            // space.
            float theta1 = atan2f(seg2->p1[1] - seg2->p0[1],
                                  seg2->p1[0] - seg2->p0[0]);
            ...
            float neighbor_cost = td->neighbor_k1*fabs(dtheta - M_PI/2) + td->neighbor_k2*sqrtf(dist2/length2) + td->neighbor_k3*sqrtf(dist2) + 1;

            if (neighbor_cost >= 0)
                zarray_add(seg->neighbors, &seg2);
        }
...
static inline double detection_radius(april_tag_detection_t *det)
{
    double r = 0;
    for (int i = 0; i < 4; i++) {
        double ri = sqrt(sq(det->c[0] - det->p[i][0]) +
                         sq(det->c[1] - det->p[i][1]));
        r += ri;
    }
    return r / 4.0;
}
...
april_tag_detection_t *quad_decode_real(april_tag_detector_t *td, april_tag_family_t *family, image_u8_t *im, struct quad *quad, struct quad_decode_task *task, uint32_t guessid)
{
        ...
        for (int ix = -1; ix <= dd; ix++) {
            ...
            if ((iy == -1 || iy == dd) || (ix == -1 || ix == dd)) {
                ...
                if (task != NULL && task->im_gray_samples)
                    task->im_gray_samples->buf[iry*task->im_gray_samples->stride + irx] = 0;
                ...
                if (task != NULL && task->im_gray_samples)
                    task->im_gray_samples->buf[iry*task->im_gray_samples->stride + irx] = 255;
            }
        }
    }
...
//            printf("%15f %15f\n", black_mean, white_mean);
    ...
    for (uint32_t iy = 0; iy < family->d; iy++) {
        for (uint32_t ix = 0; ix < family->d; ix++) {
    ...
    if (1) {//for (int tfidx = 0; tfidx < zarray_size(families); tfidx++) {
        ...
        if (decode_tag(family, tag_code, &id, &hamming, &rotation, &rotatedcode, guessid)) {
...
void quad_decode(april_tag_detector_t *td, april_tag_family_t *family, image_u8_t *im, struct quad *quad, struct quad_decode_task *task)
{
    april_tag_detection_t *det = quad_decode_real(td, family, im, quad, task, UINT32_MAX);
            ...
            for (int edge = 0; edge < 4; edge++) {
                ...
                for (int stepdir = 0; stepdir < 2; stepdir++) {
                    ...
                    for (float stepsz = .25; stepsz <= 4; stepsz *= 2) {
                        ...
                        // not needed to do a deep copy of segments... we only use the quads.
                        struct quad qcopy;
                        ...
                        april_tag_detection_t *thisdet = quad_decode_real(td, family, im, &qcopy, task, det == NULL ? UINT32_MAX : det->id);
                        if (thisdet == NULL)
                            continue;
                        ...
                        if (det == NULL ||
                            ...
                            (thisdet->hamming == det->hamming && thisdet->goodness > det->goodness)) {
...
zarray_t *april_tag_detector_detect(april_tag_detector_t *td, image_u8_t *im_orig)
{
    ...
    if (td->debug)
        image_u8_write_pgm(im_seg, "debug_blur.pnm");
    ...
    if (1) {
        ...
        int chunksize = 1 + zarray_size(segments) / (APRILTAG_TASKS_PER_THREAD_TARGET * td->nthreads);
 //       int chunksize = 10;
...
/*                for (int j = 0; j < 4; j++)
                    image_u8_draw_line(im_quads,
                                       ...
                                       quad->segments[j]->p1[0], quad->segments[j]->p1[1], 200, 1);
*/
 ...
 //       int chunksize = 5;
    ...
    if (td->debug) {
        ...
        for (int detidx = 0; detidx < zarray_size(detections); detidx++) {
            ...
            float scale = ((float[]) {1.0, 0.8, 0.6, 0.4, 0.4, 0.4, 0.4 })[det->hamming];
