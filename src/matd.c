...
matd_t *matd_inverse(const matd_t *x)
{
    ...
    case 2: {
        ...
        double invdet = 1.0 / make_non_zero(MAT_EL(x, 0, 0) * MAT_EL(x, 1, 1) - MAT_EL(x, 0, 1) * MAT_EL(x, 1, 0));
    ...
    case 3: {
        ...
        double det = make_non_zero(a*e*i-a*f*h-d*b*i+d*c*h+g*b*f-g*c*e);
        det = 1.0/det;
...
double matd_det(const matd_t *x)
{
    ...
    case 2: {
        double det = (MAT_EL(x, 0, 0) * MAT_EL(x, 1, 1) - MAT_EL(x, 0, 1) * MAT_EL(x, 1, 0));
        return det;
    }
    ...
    for (int j = 0; j < a->ncols; j++) {
        for (int i = 0; i < a->nrows; i++) {
            ...
            for (int k = 0; k < kmax; k++)
                acc += MAT_EL(lu, i, k) * MAT_EL(lu, k, j);
...
matd_t *matd_lu_solve(const matd_lu_t *mlu, const matd_t *b)
{
    ...
    for (int i = 0; i < mlu->lu->nrows; i++)
        memcpy(&MAT_EL(x, mlu->piv[i], 0), &MAT_EL(b, i, 0), sizeof(TYPE) * b->ncols);
                               ...
                               matd_t **garb, int *garbpos, int *isarg, int oneterm)
{
    while (expr[*pos] != 0) {
        ...
        case '*':
            ...
            case 'F': {
                garb[*garbpos] = args[*argpos];
                (*garbpos)++;
            } // fall through

        case 'M': {
            matd_t *rhs0 = args[*argpos];
            (*pos)++;
            (*argpos)++;

            rhs = matd_op_gobble_right(expr, pos, rhs0, garb, garbpos, isarg);
